...
@torch.no_grad()
def get_next(model, data_loader, device,namee):
    model.eval()
    ...
    label_dict = json.load(open('/home/yzbj10/Work/lxj/deep-learning-for-image-processing/pytorch_classification/mini_imagenet/classes_name.json', "r"))
    #任意的多组列表
    aaa = []
    bbb = []    
    ...
    for step, data in enumerate(data_loader):
        ...
        # cls = [key for key, value in label_dict.items() if value[0] == pred.item()]
        aaa.append(names[0])
        # bbb.append(cls[0])
        bbb.append(pred.item())
    
    #字典中的key值即为csv中列名
    dataframe = pd.DataFrame({'filename':aaa,'label':bbb})

    #将DataFrame存储为csv,index表示是否显示行名，default=True
    dataframe.to_csv("/home/yzbj10/Work/lxj/data/mini_imagenet/mini-imagenet/"+namee,index=False,sep=',')
